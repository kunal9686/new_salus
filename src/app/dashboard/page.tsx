        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          ...
          <Card className="col-span-1 md:col-span-2 clay-card animate-in slide-in-from-bottom-4 duration-500 delay-100">
            <CardHeader>
              ...
              <CardDescription>Your emotional state is the compass for our journey today.</CardDescription>
            </CardHeader>
            <CardContent>
              {!moodSubmitted ? (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {moods.map((m) => (
                    <Button
                      key={m.id}
                      variant="outline"
                      className="h-24 flex flex-col gap-2 rounded-3xl border-2 border-primary/10 hover:border-primary hover:bg-primary/10 transition-all bg-white/40"
                ...
                <div className="py-8 text-center animate-in fade-in slide-in-from-bottom-2 duration-700">
                  <div className="size-16 rounded-3xl bg-primary/20 flex items-center justify-center mx-auto mb-4 border border-primary/30">
                    <Laugh className="h-10 w-10 text-primary" />
                  </div>
                  <h3 className="text-2xl font-headline font-bold">Thank you for sharing.</h3>
                  <p className="text-muted-foreground mt-1">Self-awareness is the first step toward growth.</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="clay-card overflow-hidden group animate-in slide-in-from-bottom-4 duration-500 delay-200">
            ...
            <CardContent className="space-y-4">
              <div className="flex justify-between items-center p-4 rounded-3xl bg-primary/10 border border-white/40 shadow-inner">
              ...
              <div className="flex justify-between items-center p-4 rounded-3xl bg-accent/10 border border-white/40 shadow-inner">
                <div className="flex items-center gap-3">
          ...
          <Card className="col-span-1 md:col-span-3 lg:col-span-2 clay-card animate-in slide-in-from-bottom-4 duration-500 delay-300">
            <CardHeader>
              ...
              <CardDescription>Your inner weather over the past week.</CardDescription>
            </CardHeader>
            <CardContent className="h-[280px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData}>
                  ...
                  <Tooltip 
                    contentStyle={{ backgroundColor: 'white', border: 'none', borderRadius: '20px', boxShadow: '0 10px 25px rgba(0,0,0,0.05)' }}
          ...
          <Card className="col-span-1 md:col-span-3 lg:col-span-1 clay-card animate-in slide-in-from-bottom-4 duration-500 delay-400">
            ...
            <CardContent className="space-y-4">
              {recentEntries && recentEntries.length > 0 ? recentEntries.map((entry, idx) => (
                <div key={entry.id} className="p-4 rounded-3xl bg-white/40 border border-white/60 hover:bg-white/60 transition-all cursor-pointer group animate-in fade-in slide-in-from-right-4 duration-500 shadow-sm" style={{ animationDelay: `${500 + (idx * 100)}ms` }}>
                  ...
                  <p className="text-sm line-clamp-3 leading-relaxed text-foreground/80">{entry.content}</p>
                </div>
              )) : (
                <div className="py-12 text-center text-muted-foreground italic animate-in fade-in duration-1000">
