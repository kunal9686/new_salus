        ...
        <div className="max-w-3xl mx-auto space-y-12">
          <div className="space-y-4 text-center">
            <div className="size-16 rounded-[2.5rem] bg-accent/20 flex items-center justify-center mx-auto border-4 border-white shadow-xl animate-in zoom-in duration-700">
              <UserCircle2 className="size-8 text-accent-foreground" />
            </div>
            <h2 className="text-3xl font-headline font-bold text-foreground">Reclaim Your Narrative</h2>
          ...
          <div className="space-y-8">
            {prompts.map((p, idx) => (
              <Card key={p.id} className="clay-card animate-in slide-in-from-bottom-6 duration-500" style={{ animationDelay: `${idx * 150}ms` }}>
                <CardHeader className="p-10 pb-4">
                  <CardTitle className="text-lg font-headline text-primary">{p.label}</CardTitle>
                </CardHeader>
                <CardContent className="p-10 pt-0">
                  <Textarea 
                    placeholder={p.placeholder} 
                    className="min-h-[150px] bg-white/40 border-2 border-white rounded-[2rem] p-6 text-sm"
            ...
            <Button onClick={handleSubmit} className="w-full h-20 text-lg font-headline rounded-[2.5rem] group clay-btn shadow-2xl" disabled={isSubmitting}>
              Seal Reflection
